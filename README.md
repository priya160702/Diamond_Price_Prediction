...
Price (Target): The price in US dollars ($326–$18,823).

### Data Preprocessing

To ensure model accuracy, the following steps were performed:

Data Cleaning: Removed the unnecessary index column (Unnamed: 0).

Outlier Removal: Filtered out "dimensionless" diamonds where x, y, or z values were zero.

Feature Encoding: Applied LabelEncoder and OneHotEncoder to convert categorical text data into numerical formats.

Scaling: Standardized features using StandardScaler.

Dimensionality Reduction: Utilized Principal Component Analysis (PCA) to simplify the dataset.
...
XGBoost Regressor (Final model used for prediction)
